...
function fetchUserData() {
  return fetch('http://6735-193-151-58-83.ngrok-free.app/users/')
    .then(response => response.json())
    .then(data => {
      user = data; // Assign the retrieved user data to the user variable
    });
}
...
Promise.all([fetchUserData(), fetchGradesData(), fetchSubjectsData() ])
  .then(() => {
    ...
    const userGrades = grades.filter(grade => grade.student === 2);
